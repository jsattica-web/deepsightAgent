...
PowerShell 기준으로 작성했으며, 명령어는 프로젝트 루트 또는 `agent-python` 폴더에서 실행할 수 있도록 구분했다.
...
```powershell
D:\eclipse\workspace\deepsightAgent
```
...
판매, 수주, 재고 Tool 함수가 각각 정상 동작하는지 확인한다.
...
이 테스트도 mock을 사용하므로 실제 DB 연결이 필요 없다.
...
DB 연결이 정상이라면 `database` 값이 `connected`로 나온다.

## 10. Agent Chat API 테스트

Agent가 자연어 질문을 받아 적절한 Tool을 선택하는지 확인한다.
...
Agent를 거치지 않고 Tool API만 직접 확인할 때 사용한다.
...
전체 테스트 실행 중 아래와 같은 로그가 보일 수 있다.
...
FastAPI TestClient 관련 경고이며 현재 테스트 실패 원인은 아니다.
